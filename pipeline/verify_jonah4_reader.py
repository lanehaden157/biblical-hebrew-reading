"""
Independent verification of data/jonah4_reader.json (rule 3).

Sibling of verify_jonah1_reader.py / verify_jonah2_reader.py /
verify_jonah3_reader.py -- deliberately does NOT import
build_jonah4_reader.py. Re-scans the raw WLC XML with its own regex,
re-derives its own gloss lookup from vocab_deck_600.json plus all four
Jonah extras, and recomputes every word's surface form, transliteration,
gloss and is_known flag from scratch, then diffs against the shipped JSON
entry-by-entry.

Checks performed: same six as verify_jonah1_reader.py, scoped to Jonah 4's
11 verses.
"""
import json
import os
...
DATA_PATH = os.path.join(HERE, "..", "data", "jonah4_reader.json")

CANTILLATION_RE = re.compile("[֑-֯]")
ALLOWED_HEB_RE = re.compile("^[ְ-ׇא-ת]*$")
ALLOWED_TRANSLIT_RE = re.compile(r"^['`a-z]+$")

VERSE_RE = re.compile(r'<verse osisID="(Jonah\.4\.\d+)">(.*?)</verse>', re.S)
...
def rescan(lookup, known_ids):
    """Independent regex pass over Jonah.xml, restricted to chapter 4."""
...
def main():
    if not os.path.isfile(DATA_PATH):
        sys.exit("data/jonah4_reader.json not found -- run pipeline/build_jonah4_reader.py first")
    ...
    print("Re-scanning Jonah 4 independently (regex-based)...")
