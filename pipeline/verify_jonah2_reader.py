"""
Independent verification of data/jonah2_reader.json (rule 3).

Sibling of verify_jonah1_reader.py -- deliberately does NOT import
build_jonah2_reader.py. Re-scans the raw WLC XML with its own regex,
re-derives its own gloss lookup from vocab_deck_600.json + jonah1_extra.json
+ jonah2_extra.json, and recomputes every word's surface form,
transliteration, gloss and is_known flag from scratch, then diffs against
the shipped JSON entry-by-entry. Imports transliterate() directly (that
module has its own separate verify_transliterate.py).

Checks performed: same six as verify_jonah1_reader.py, scoped to Jonah 2's
11 verses instead of Jonah 1's 16.
"""
import json
import os
import re
...
DATA_PATH = os.path.join(HERE, "..", "data", "jonah2_reader.json")

CANTILLATION_RE = re.compile("[֑-֯]")
ALLOWED_HEB_RE = re.compile("^[ְ-ׇא-ת]*$")
ALLOWED_TRANSLIT_RE = re.compile(r"^['`a-z]+$")

VERSE_RE = re.compile(r'<verse osisID="(Jonah\.2\.\d+)">(.*?)</verse>', re.S)
...
def load_gloss_lookup():
    ...
    for path in (EXTRA1_PATH, EXTRA2_PATH):
...
def rescan(lookup, known_ids):
    """Independent regex pass over Jonah.xml, restricted to chapter 2."""
...
def main():
    if not os.path.isfile(DATA_PATH):
        sys.exit("data/jonah2_reader.json not found -- run pipeline/build_jonah2_reader.py first")
    ...
    print("Re-scanning Jonah 2 independently (regex-based)...")
