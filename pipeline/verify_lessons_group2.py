"""
Independent verification of data/lessons_group2.json (suggestion 3).

Deliberately does NOT import build_lessons_group1.decompose() or
build_lessons_group2.noun_role() -- re-implements both from scratch (same
independence discipline as verify_lessons_group1.py) against a fresh XML
re-scan, then diffs every field of every example (single-word and chain
alike) against the shipped JSON.
...
Checks performed:
  ...
  5. Chain-specific: every token's grammatical role (construct/absolute)
     is independently re-derived from its own raw morph code's state
     letter, not trusted from the shipped JSON or from word order, and a
     chain's tokens must all share one verse ref.
...
DATA_PATH = os.path.join(HERE, "..", "data", "lessons_group2.json")
...
# Word ids, keyed the same way build_lessons_group2.py curates them -- the
# curatorial choice of which words illustrate a concept is trusted (see
# docstring); what's checked is whether they were decomposed, classified,
# and glossed correctly.
CHAIN_WORD_IDS = {
    "basic-chain": [["32Nvk", "32TyA"], ["32k5P", "32nPh"]],
    "definiteness": [["32Cof", "32YWU"], ["32Sxc", "32xwN"]],
    "three-chain": [["32xkK", "32rGa", "32fp1"]],
}
SINGLE_WORD_IDS = {
    "shrinks": ["32Nvk", "32Ew6", "32Sxc"],
    "special-endings": ["32GUh", "32Cof", "32HoD"],
}
BOOK = "Jonah"
...
def decompose(raw_lemma, raw_morph, raw_text, by_id):
    """Independent re-implementation (same as verify_lessons_group1.py's,
    duplicated rather than imported -- see that file's own docstring for
    why -- plus the content word's own morph part and derived role)."""
    ...
    content_morph = morph_parts[n_prefix]
    m = STATE_RE.match(content_morph)
    role = ("construct" if m.group(1) == "c" else "absolute") if m else "absolute"
...
def main():
    if not os.path.isfile(DATA_PATH):
        sys.exit("data/lessons_group2.json not found -- run pipeline/build_lessons_group2.py first")
        ...
        if lesson["kind"] == "chain":
            ...
            for wids, ex in zip(expected_groups, lesson["examples"]):
                ...
                for wid, tok in zip(wids, tokens):
                    ...
                    if tok.get("role") != expected["role"]:
                        fail(f"{tag}/{wid}: role mismatch: shipped {tok.get('role')!r}, recomputed {expected['role']!r}")
                if len(refs) != 1:
