"""
Independent verification of data/lessons_group5.json (suggestion 3).

Deliberately does NOT import build_lessons_group1.decompose() or
build_lessons_group5's own builders -- re-implements the decomposition
...
Checks performed:
  ...
  5. Chain-specific: every token's role is "plain" (word-order and
     verbless-clause examples are read as an ordered sequence, not a
     construct chain -- there's no construct/absolute lean to classify),
     and a chain's tokens must all share one verse ref.
...
DATA_PATH = os.path.join(HERE, "..", "data", "lessons_group5.json")
...
CHAIN_WORD_IDS = {
    "word-order": [["32v8V", "32T2S", "32tkJ", "32EXL"], ["32A4a", "325Pr"]],
    "verbless-clauses": [["32TXw", "32Y4K"], ["32ZMk", "32irW", "32x72", "32FYV"]],
}
SINGLE_WORD_IDS = {
    "vav-connector": ["32A4a", "32d5m"],
}
BOOK = "Jonah"
