"""
Phase 5 step 6: OSHB corpus -> word-aligned Jonah 4 reading data ->
data/jonah4_reader.json.

Sibling of build_jonah1_reader.py / build_jonah2_reader.py /
build_jonah3_reader.py -- same word-per-printed-word approach, same "no
grading, no scheduler state, no per-word part of speech" design.

Gloss sources: data/vocab_deck_600.json plus all four Jonah extras
(chapters 1-4; glosses/jonah4_extra.json is curated by
pipeline/curate_jonah4_extra.py for the 17 lemmas genuinely new to this,
the final, chapter). An unresolved lemma is a hard error (rule 4).

Chapter scope: 11 verses (Jonah.4.1-Jonah.4.11). No Hebrew/English
versification divergence in this chapter.

Run pipeline/curate_jonah4_extra.py first (needs glosses/jonah4_extra.json).
"""
import json
import os
import re
...
EXTRA_PATHS = [
    ...
    os.path.join(HERE, "..", "glosses", "jonah4_extra.json"),
]
OUT_PATH = os.path.join(HERE, "..", "data", "jonah4_reader.json")

CANTILLATION_RE = re.compile("[֑-֯]")
EXPECTED_VERSE_COUNT = 11
CHAPTER_PREFIX = "Jonah.4."
...
def main():
    ...
    if len(verses) != EXPECTED_VERSE_COUNT:
        print(
            f"NOTE: found {len(verses)} verses in Jonah 4, expected {EXPECTED_VERSE_COUNT}. "
    ...
    out = {
        "metadata": {
            ...
            "chapter": "Jonah 4",
            "versification": "Hebrew/WLC (11 verses; no divergence from English here)",
